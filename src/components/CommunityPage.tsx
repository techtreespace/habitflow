  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-5 px-5 snap-x snap-mandatory no-scrollbar">
          {GROUPS.map((group, i) => (
            <motion.div
              ...
              onClick={() => group.active && setSelectedGroup(group)}
            >
          ...
          {SAMPLE_FEED.map((post, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">{post.user}</span>
                <div>
      ...
      <motion.div
        ...
        className="bg-gradient-to-br from-accent/10 to-primary/10 rounded-xl p-4 border border-accent/20"
      >
        <div className="flex items-center gap-2 mb-2.5">
          <Crown className="w-4 h-4 text-accent" />
          <h3 className="text-xs font-bold">Pro로 커뮤니티에 참여하세요</h3>
        </div>
        <div className="space-y-2">
          {[
            { icon: PenLine, text: "습관 피드 글쓰기 & 공유" },
            { icon: Heart, text: "좋아요 & 댓글로 응원하기" },
            { icon: TrendingUp, text: "나와 비슷한 사용자 달성률 비교" },
            { icon: MessageCircle, text: "그룹 채팅 & 1:1 습관 버디 매칭" },
          ].map(({ icon: Icon, text }) => (
            <div key={text} className="flex items-center gap-2.5">
              <Icon className="w-3.5 h-3.5 text-primary" />
              <p className="text-[11px] text-muted-foreground flex-1">{text}</p>
              <Lock className="w-3 h-3 text-muted-foreground" />
            </div>
          ))}
        </div>
        <button className="mt-3 w-full py-2 rounded-lg bg-accent text-accent-foreground font-semibold text-xs">
          Pro 업그레이드 →
        </button>
      </motion.div>
    </div>
